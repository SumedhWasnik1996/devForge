...
export default function NewWorkspaceModal({ open, onClose, onCreated }: Props) {
    ...
    useEffect(() => {
        if (!open) return;
        // Load all accounts when modal opens
        Promise.all([
    ...
    // When jira account is selected, load boards from stories
    useEffect(() => {
    ...
    const reset = () => {
        setStep(0); setError(""); setWorkspaceName(""); setJiraAccountId(null);
        setJiraBoard(""); setGithubAccountId(null); setGitRepoUrl("");
        setGitBranch("main"); setSfAccountId(null);
    };
                        ...
                        <Box>
                            ...
                            {jiraAccounts.length === 0 ? (
                                <Alert severity="warning" sx={{ fontSize: 12 }}>
                                    No Jira accounts connected. Go to Settings → Connections.
                                </Alert>
                            ) : (
                                jiraAccounts.map(acc => (
                                    <Box
                                        key={acc.id}
                                        onClick={() => setJiraAccountId(acc.id)}
                                        sx={{
                                            border: "1px solid",
                                            borderColor: jiraAccountId === acc.id
                                                ? "primary.main" : "divider",
                                            borderRadius: 1.5, p: 1.5, mb: 1,
                                            cursor: "pointer", display: "flex",
                                            alignItems: "center", gap: 1.5,
                                            bgcolor: jiraAccountId === acc.id
                                                ? "primary.50" : "transparent",
                                        }}
                                    >
                                        {acc.avatar_url && (
                                            <img src={acc.avatar_url} width={24}
                                                height={24} style={{ borderRadius: "50%" }} />
                                        )}
                                        <Box>
                        ...
                        {jiraAccountId && boards.length > 0 && (
                            <Box>
                                ...
                                {boards.map(board => (
                                    <Box
                                        key={board}
                                        onClick={() => setJiraBoard(board)}
                                        sx={{
                                            border: "1px solid",
                                            borderColor: jiraBoard === board
                                                ? "primary.main" : "divider",
                                            borderRadius: 1.5, p: 1.5, mb: 1,
                                            cursor: "pointer", display: "flex",
                                            justifyContent: "space-between",
                                            alignItems: "center",
                                            bgcolor: jiraBoard === board
                                                ? "primary.50" : "transparent",
                                        }}
                                    >
                    ...
                    <Box display="flex" flexDirection="column" gap={2}>
                        <Box>
                            ...
                            {githubAccounts.map(acc => (
                                <Box
                                    key={acc.id}
                                    onClick={() => setGithubAccountId(
                                        githubAccountId === acc.id ? null : acc.id
                                    )}
                                    sx={{
                                        border: "1px solid",
                                        borderColor: githubAccountId === acc.id
                                            ? "primary.main" : "divider",
                                        borderRadius: 1.5, p: 1.5, mb: 1,
                                        cursor: "pointer", display: "flex",
                                        alignItems: "center", gap: 1.5,
                        ...
                        <TextField
                            label="Repository URL" size="small" fullWidth
                            value={gitRepoUrl}
                            onChange={e => setGitRepoUrl(e.target.value)}
                            placeholder="https://github.com/org/repo.git"
                        />
                        <TextField
                            label="Branch" size="small" fullWidth
                            value={gitBranch}
                    ...
                    <Box display="flex" flexDirection="column" gap={2}>
                        ...
                        {sfAccounts.length === 0 ? (
                            ...
                            sfAccounts.map(acc => (
                                <Box
                                    key={acc.id}
                                    onClick={() => setSfAccountId(
                                        sfAccountId === acc.id ? null : acc.id
                                    )}
                                    sx={{
                                        border: "1px solid",
                                        borderColor: sfAccountId === acc.id
                                            ? "primary.main" : "divider",
                ...
                <Box display="flex" gap={1}>
                    ...
                    {step === 2 && (
                        <Button size="small" variant="text"
                            color="inherit" onClick={() => setStep(3)}>
                            Skip for now
                        </Button>
                    )}
                    {step === 2 && sfAccountId && (
                        <Button size="small" variant="contained"
                            disableElevation onClick={() => setStep(3)}>
                            Next →
                        </Button>
                    )}
                    {step === 3 && (
