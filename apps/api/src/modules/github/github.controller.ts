...
import {
    ...
    Query, Res, Param, ParseIntPipe, Body,
} from "@nestjs/common";
...
export class GithubController {
    ...
    async cb(@Query("code") code: string, @Res() res) {
        ...
        res.send(`
            <!DOCTYPE html><html><head><title>DevForge - GitHub Connected</title></head>
            <body style="font-family:sans-serif;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;background:#f5f5f5;">
                <div style="text-align:center;padding:2rem;background:white;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,0.1);">
                    ...
                    <button onclick="window.close()" style="padding:10px 24px;background:#24292f;color:white;border:none;border-radius:6px;font-size:14px;cursor:pointer;">Close Window</button>
                </div>
    ...
    @Get("status")
    status() { return this.github.status(); }

    @Get("accounts")
    accounts() { return this.github.getAccounts(); }
     ...
     * Body: { name: string; accountId?: number; private?: boolean }
     * Creates a new GitHub repo pre-populated with the SDFX scaffold.
     */
    @Post("repos/create")
    createRepo(
        @Body() body: { name: string; accountId?: number; private?: boolean },
    ) {
        return this.github.createRepo(
            body.name,
            body.accountId,
            body.private ?? true,
        );
    ...
    /**
     * GET /github/repos/:owner/:repo/tree?branch=main&accountId=1
     */
    ...
    repoTree(
        ...
        return this.github.getRepoTree(
            owner, repo,
            branch || "main",
            accountId ? parseInt(accountId, 10) : undefined,
        );
    ...
    /**
     * GET /github/repos/:owner/:repo/file?path=src/index.ts&branch=main&accountId=1
     */
    ...
    /**
     * GET /github/repos/:owner/:repo/pulls?state=open&accountId=1
     */
    ...
    pullRequests(
        ...
        return this.github.getPullRequests(
            owner, repo,
            state || "open",
            accountId ? parseInt(accountId, 10) : undefined,
        );
    ...
    /**
     * GET /github/repos/:owner/:repo/branches?accountId=1
     */
